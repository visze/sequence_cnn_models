...
__all__ = ['StringClassificationDataLoader1D', 'SeqClassificationDataLoader1D']
...
class StringClassificationDataLoader1D(Dataset):
    """
    info:
        doc: >
...
@kipoi_dataloader(override={"dependencies": deps, 'info.authors': package_authors})
class SeqClassificationDataLoader1D(Dataset):
    """
    info:
        doc: >
...
@kipoi_dataloader(override={"dependencies": deps, 'info.authors': package_authors})
class StringRegressionDataLoader1D(Dataset):
    """
    info:
        doc: >
            Dataloader for a tab-delimited input file containing in the first column the sequence and the second the class label.
    ...
    def __getitem__(self, idx):
        ...
        if self.ignore_targets:
            ...
            labels = row.iloc[2:].values.astype(self.label_dtype)
...
@kipoi_dataloader(override={"dependencies": deps, 'info.authors': package_authors})
class SeqRegressionDataLoader1D(Dataset):
    """
    info:
        doc: >
            Dataloader for a combination of fasta and tab-delimited input files such as bed files. The dataloader extracts
    ...
    def __init__(self,
        ...
        self.seq_dl = StringRegressionDataLoader1D(tsv_file, label_dtype=label_dtype,
                                                   label_start_column=label_start_column, ignore_targets=ignore_targets)
